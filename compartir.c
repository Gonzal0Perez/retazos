...
#include <sys/stat.h>
#include <sys/shm.h> 
int main () {int segment_id; char* shared_memory; struct shmid_ds shmbuffer;pid_t segment_owner,segment_buyer;
  int segment_size; int next[2]; const int shared_segment_size = 0x6400;/*Allocate*/ 
  segment_id=shmget(IPC_PRIVATE,shared_segment_size,IPC_CREAT | IPC_EXCL | S_IRUSR | S_IWUSR);
  /*size*/shmctl(segment_id,IPC_STAT,&shmbuffer);
  ...
  int *p ;sprintf(shared_memory," %s \n","Hello, world.");printf("Jaaaa \n");
  printf("sharing %d by %d \n",segment_id,segment_owner ) ; //p=arbo1->s+offsetof(tipoNod0,s);
