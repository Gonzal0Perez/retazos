...
{  ntests += 1;
  ...
  { struct bn a, b, c;
    ...
    bignum_div(&b, &a, &c); }/* test passed if it doesn't go into infinite-loop... */
  npassed += 1;
}
...
{ ntests += 1;
  ...
  { struct bn a;struct bn b;struct bn c;
    bignum_from_int(&a, 0);  bignum_from_int(&b, 1);
    bignum_sub(&a, &b, &a); bignum_from_int(&b, 3);
    bignum_add(&a, &b, &a); bignum_from_int(&c, 2);
...
int main(){ printf("\nRunning hand-picked test cases:\n");
  test_evil();test_over_and_underflow();
