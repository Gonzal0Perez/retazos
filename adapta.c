/******************************************************************************
        ...
        Application:  Autoassociative Memory, Associative Recall of Images and Pattern Completion
        Author:       Karsten Kutza  /  Date:         30.1.96
...
void InitializeApplication(NET* Net){INT n,i,j;
  for (n=0; n<NUM_DATA; n++) {
    for (i=0; i<Y; i++) {
      for (j=0; j<X; j++) {Input [n][i*X+j] = BIPOLAR(Pattern [n][i][j] == '0');
                           Input_[n][i*X+j] = BIPOLAR(Pattern_[n][i][j] == ' '); ;} } }
  f = fopen("HOPFIELD.txt", "w");}

void WriteNet(NET* Net){INT i,j;   
  for (i=0; i<Y; i++) {for (j=0; j<X; j++) {fprintf(f, "%c", BINARY(Net->Output[i*X+j]) ? 'O' : ' '); ;}
                       fprintf(f, "\n");} fprintf(f, "\n");}
 
void FinalizeApplication(NET* Net){fclose(f);}
/*************************I N I T I A L I Z A T I O N                         */
void GenerateNetwork(NET* Net){INT i;Net->Units     = N;
...
void CalculateWeights(NET* Net){INT i,j,n;INT Weight;
  for (i=0; i<Net->Units; i++) {
    for (j=0; j<Net->Units; j++) {Weight = 0;
                                  if (i!=j) {for (n=0; n<NUM_DATA; n++) {Weight += Input[n][i] * Input[n][j];} }
                                  Net->Weight[i][j] = Weight;} }}

void SetInput(NET* Net, INT* Input){INT i;
  for (i=0; i<Net->Units; i++) {Net->Output[i] = Input[i];}
  WriteNet(Net);}

void GetOutput(NET* Net, INT* Output){INT i;   
  for (i=0; i<Net->Units; i++) {Output[i] = Net->Output[i];}
  WriteNet(Net);}

/******************* P R O P A G A T I N G   S I G N A L S                    */
BOOL PropagateUnit(NET* Net, INT i){INT  j;INT  Sum, Out;BOOL Changed; Changed = FALSE;Sum = 0;
  for (j=0; j<Net->Units; j++) {Sum += Net->Weight[i][j] * Net->Output[j];}
  if (Sum != Net->Threshold[i]) {if (Sum < Net->Threshold[i]) Out = LO;
                                 if (Sum > Net->Threshold[i]) Out = HI;
                                 if (Out != Net->Output[i]) {Changed = TRUE;Net->Output[i] = Out;} }
  return Changed;}

void PropagateNet(NET* Net){INT Iteration, IterationOfLastChange; Iteration = 0;IterationOfLastChange = 0;
  do {Iteration++;
      if (PropagateUnit(Net, RandomEqualINT(0, Net->Units-1)))IterationOfLastChange = Iteration;
...
int main(){NET Net; INT n;
  InitializeRandoms(); GenerateNetwork(&Net);InitializeApplication(&Net);CalculateWeights(&Net);   
  for (n=0; n<NUM_DATA; n++) {SimulateNet(&Net, Input[n]);}
  for (n=0; n<NUM_DATA; n++) {SimulateNet(&Net, Input_[n]);}
